...
import { validateCustomName, validateDescription, writeMetadata } from '../metadataService';
import { 
  fileExists, 
  writeServerFile, 
 ...
 */
async function validateServerCreation(data: CreateServerData): Promise<{ valid: boolean; error?: string }> {
  if (data.customName) {
 ...
 */
async function setupServerDirectory(dateString: string): Promise<{ serverPath: string; exists: boolean }> {
 ...
 */
async function extractServerFiles(zipFilePath: string, serverPath: string): Promise<void> {
