...
export async function findStartFiles(req: Request): Promise<Response> {
  try {
    ...
    const status = errorMessage.includes("required") ? 400 : 
 ...
 */
async function updateMetadata(project: string, startFile: string): Promise<void> {
...
export async function setStartFile(req: Request): Promise<Response> {
  try {
    ...
    await updateMetadata(project, startFile);
